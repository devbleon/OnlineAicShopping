		...
		while ($row = mysqli_fetch_array($run_query)) {
			...
			echo "</div>
										<div class='product-btns'>
											<button pid='$pro_id' id='wishlist' class='add-to-wishlist'><i class='fa fa-heart-o'></i><span class='tooltipp'>Agregar a la lista deseos</span></button>
											<button class='add-to-compare'><i class='fa fa-exchange'></i><span class='tooltipp'>Agregar para comparar</span></button>
											<button class='quick-view'><i class='fa fa-eye'></i><span class='tooltipp'>Visa rápida</span></button>
										</div>
									</div>
...
if (isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])) {
	...
	while ($row = mysqli_fetch_array($run_query)) {
		...
		echo "</div>
										<div class='product-btns'>
											<button pid='$pro_id' id='wishlist' class='add-to-wishlist' tabindex='0'><i class='fa fa-heart-o'></i><span class='tooltipp'>Agregar a lista de deseos</span></button>
											<button class='add-to-compare'><i class='fa fa-exchange'></i><span class='tooltipp'>Agregar para comparar</span></button>
		...
		if ($count > 0) {
			echo "
				<div class='alert alert-warning'>
						...
						<b>El producto ya esta añadido al carrito, Continuar comprando ..!</b>
				</div>
			"; //not in video
		} else {
			...
			if (mysqli_query($con, $sql)) {
				...
				if (mysqli_query($con, $sql)) {
					echo "<div class='alert alert-danger'>
									<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
									<b>El producto fue removido de la lista de favoritos y se agrega al carrito ..!</b>
							</div>";
				}
			}
		}
	...
	if (isset($_SESSION["uid"])) {
		...
		if ($count > 0) {
			echo "
				<div class='alert alert-warning'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>El producto ya está agregado a la lista de favoritos, Continuar comprando ..!</b>
				</div>
			"; //not in video
		} else {
			...
			if (mysqli_query($con, $sql)) {
				...
				if (mysqli_query($con, $sql)) {
					echo "<div class='alert alert-danger'>
									<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
									<b>El producto se elimina del carrito y se agrega a la lista de favoritos</b>
							</div>";
				}
			}
		}
		...
		if (mysqli_num_rows($query) > 0) {
			echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>El producto ya está agregado a la lista de deseos, Continuar comprando ..!</b>
					</div>";
			exit();
		}
		$sql = "INSERT INTO `wishlist`
		...
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row = mysqli_fetch_array($query)) {
				...
				'
						...
						<tr>
							...
							<td>
								<a href="#" id="wishlist" pid="' . $product_id . '" class="btn btn-warning">Mover a lista de favoritos <i class="fa fa-angle-right"></i> </a>
							</td>
						</tr>
			...
			echo '</tbody>
				<tfoot>
					...
					<tr>
						<td><a href="store.php" class="btn btn-warning"><i class="fa fa-angle-left"></i> Continuar Comprando</a></td>
			...
			if (!isset($_SESSION["uid"])) {
				echo '
							...
							<a href="signup_form.php" class="btn btn-success">Lista para Pagar</a></td>
								</tr>
							</tfoot>
			...
			while ($row = mysqli_fetch_array($query)) {
				...
				'
						...
						<tr>
							...
							<td class="actions" data-th="">
							<a href="#" id="product" pid="' . $product_id . '" class="btn btn-success">Mover al carrito</a>
							</td>
						</tr>
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if (mysqli_query($con, $sql)) {
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>El producto fue removido del carrito</b>
				</div>";
		exit();
	}
}
...
if (isset($_POST["removeItemFromwishList"])) {
	...
	if (mysqli_query($con, $sql)) {
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>El producto fue removido de la lista de favoritos</b>
				</div>";
		exit();
	}
}
...
if (isset($_POST["updateCartItem"])) {
	...
	if (mysqli_query($con, $sql)) {
		echo "<div class='alert alert-info'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>El producto está acualizado</b>
				</div>";
		exit();
	}
}
